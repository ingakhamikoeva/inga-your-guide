...
const PLATEAU_KEYWORDS = ["вес стоит","вес не уходит","вес не двигается","вес встал","вес застрял","стою на месте","плато","не худею","вес не меняется","ничего не меняется","вес топчется"];
...
Про поздние приёмы пищи и отёки: если приём пищи происходит примерно в течение 2 часов до сна — убирай из него углеводы (включая фрукты/ягоды), даже если формально это не "вечерний перекус", а обычный ужин. Если ужин происходит заметно раньше (например, около 19:00) и пользователь не упоминал отёки или прибавку от вечерних углеводов — фрукты/ягоды после него по правилу 5 допустимы, это НЕ противоречие. Если пользователь упоминает склонность к отёкам — на поздний приём пищи убирай не только углеводы, но и продукты с высоким содержанием воды (арбуз, дыня, огурцы, помидоры), а также творог, рыбу и морепродукты — они тоже могут провоцировать отёк у чувствительных людей.
...
Из клетчатки предлагай: Свежие или ферментированные овощи, грибы в любом виде. Свекла в любом виде до 18:00 часов считается клетчаткой, после 18:00 - углеводом. Яичные желтки не предлагаем (только немного в выпечке) - так как в белке 40 калорий, в желтке - 350. А задача питания по методу - снизить калорийность с помощью легких версий привычных блюд.
8. Режим питания
...
function promptFor(route, tone, overrides, ctx, day, consultationPitchEligible) {
  ...
  if (override) return `${override}\n\n${baseUserBlock(ctx, day)}${pitchNote}`;

  const head = `${tone}\n\n${baseUserBlock(ctx, day)}\n\n`;
  switch (route) {
    case "food_recommendation":
      return head + `МЕТОД «ЛЁГКАЯ ЗАМЕНА». Не предлагай жидкие калории (кефир, смузи, соки, капучино, латте, какао, молочный коктейль, сладкие напитки) как перекус или "лёгкое". Перекус = творог 0%, густой йогурт без сахара, белковый омлет, овощи + нежирный белок, ягоды с белком.` + pitchNote;
    case "support":
      return head + `РЕЖИМ ПОДДЕРЖКИ. 1) Отрази чувства. 2) Сними вину. 3) Один маленький шаг сейчас. Без длинных лекций, без подсчёта калорий.` + pitchNote;
    case "safety":
      return head + `РЕЖИМ БЕЗОПАСНОСТИ. Спокойно прояви заботу, рекомендуй очно к врачу, при острых симптомах — скорая. В ЭТОМ ответе НИ ОДНОГО совета про еду, БЖУ, порции, перекусы или рецепты — даже если пользователь их прямо просит. Только забота и направление к врачу.`;
    case "food_analysis":
      return head + `РЕЖИМ РАЗБОРА РАЦИОНА. Без оценок "хорошо/плохо". Подсветь 1–2 сильные стороны и одно мягкое улучшение.` + pitchNote;
    case "fixation":
      return head + `ЭТАП ФИКСАЦИИ. Поддерживай стабильность 2–4 недели, минимум жидких калорий.` + pitchNote;
    case "maintenance":
      return head + `ЭТАП УДЕРЖАНИЯ. Колебания ±1–2 кг — норма. Фокус на привычках.` + pitchNote;
    default:
      return head + `ОБЩИЙ РЕЖИМ. По сути вопроса. Не предлагай жидкие калории как перекус.` + pitchNote;
  }
}
...
export async function handleAskInga(req, res) {
  ...
  try {
    ...
    // «Экологичная продажа»: минимум 3 недели с регистрации (успело сложиться
    // доверие) и не чаще раза в 2 месяца — обе цифры проверяем по базе, а не
    // надеемся, что модель сама "помнит" — она не помнит, каждый запрос стартует
    // с нуля.
    let consultationPitchEligible = false;
    if (looksLikePlateauFrustration(message)) {
      try {
        const { rows } = await pool.query(
          `SELECT created_at, last_consultation_pitch_at FROM public.app_credentials WHERE user_id = $1`,
          [auth.authId]
        );
        const row = rows[0];
        if (row) {
        ...
        console.warn("consultation pitch eligibility check failed:", e.message);
      }
    }

    const systemPrompt = promptFor(route, tone, overrides, ctx, day, consultationPitchEligible);
