...
function analyzeMealStructure(meal: string): string | null {
  ...
  if ((hasFastCarb || hasSweet) && !hasProtein && !hasVeg && !hasComplexCarb) {
    return 'это в основном быстрые углеводы — такой приём пищи быстро даёт энергию, но плохо держит сытость и часто усиливает голод позже. Чтобы он работал на снижение веса, добавь нежирный белок (курицу, рыбу, яйцо, творог) и овощи';
  }

  if (!hasProtein && isMainMeal(meal)) {
    return 'не видно нормального белка — без него сытость держится недолго. Добавь курицу, рыбу, яйцо, творог или нежирное мясо';
  }

  if (!hasVeg && isMainMeal(meal) && !hasComplexCarb) {
    return 'не видно овощей или клетчатки — попробуй добавить салат, тушёные овощи или зелень, чтобы приём пищи лучше насыщал';
  }
...
export function analyzeDailyNutrition(meals: string[], sex: UserSex): DailyNutritionAnalysis {
  ...
  if (mealIssues.length > 0) {
    steps.push('в каждый основной приём пищи завтра добавь нежирный белок (курица, рыба, яйцо, творог) и овощи — это держит сытость и помогает не тянуться к лишнему');
  }
  if (!hasProtein || weakFirstHalf) steps.push('завтра начни с белкового завтрака: яйца, творог, омлет, рыба или курица');
