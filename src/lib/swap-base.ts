...
// Тексты подсказки в дневнике — все утверждены Ингой 16.07.2026
// (кнопки «Покажи замену»/«Не сейчас» — ранее, в базе 38 пар; кнопка «Понятно» — в DailyScreen).
export const HINT_TEXTS = {
  teaser: 'У этого блюда есть лёгкая версия',
  title: 'Лёгкая замена',
  showButton: 'Покажи замену',
  laterButton: 'Не сейчас',
  savedLabel: (kcal: number) => `порция ≈ на ${kcal} ккал легче`,
};
