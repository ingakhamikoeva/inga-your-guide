  ...
  const runSearch = async (raw: string) => {
    ...
    if (res.kind === 'known' || res.kind === 'unknown') {
      ...
      const { answer } = await askInga({
        message: `Пользователь любит блюдо «${dishLabel}» и хочет его лёгкую версию. Обращайся к пользователю только на «вы». Подскажи в 2-4 предложениях, как приготовить это блюдо заметно легче по калориям, сохранив вкус: какие ингредиенты заменить и как готовить. По методу лёгкой замены, без запретов.`,
