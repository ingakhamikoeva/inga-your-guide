...
function detectRoute(message: string, ctx?: SafeUserContext): RouteType {
  ...
  const food = [
    ...
    "что выбрать", "чем перекусить", "что приготовить", "помоги выбрать",
  ];
...
- На вопрос "Что съесть?" главный ответ — ПОЛНОЦЕННЫЙ ПРИЁМ ПИЩИ, а НЕ напиток.
- НЕ предлагай как основной ответ: кефир, питьевой йогурт, сок, капучино, сладкие напитки и любые жидкие калории.
- Напиток не заменяет еду. Если пользователь спрашивает "можно кефир?" — мягко объясни, что это жидкие калории и лучше собрать нормальный приём.
- Предлагай тарелку: белок + овощи + немного сложных углеводов. Подстраивайся под метод (palm/plate/calories).
- Сладкое — только как "сладкая точка" сразу после еды, не вместо неё.`;
}
